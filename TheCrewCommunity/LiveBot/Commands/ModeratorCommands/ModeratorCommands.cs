...
public class ModeratorCommands(IDbContextFactory<LiveBotDbContext> dbContextFactory, IDatabaseMethodService databaseMethodService, IModeratorWarningService warningService, IModeratorLoggingService moderatorLoggingService, IModMailService modMailService, InteractivityExtension interactivity)
{
    ...
    [Command("unwarn"), Description("Removes a warning from the user"), RequirePermissions(DiscordPermission.KickMembers)]
    ...
    [Command("addnote"), Description("Add a note to the user"), RequirePermissions(DiscordPermission.ModerateMembers)]
    ...
    [Command("deletenote"), Description("Delete a note from the user."), RequireGuild, RequirePermissions(DiscordPermission.ModerateMembers)]
    ...
    [Command("editnote"), Description("Edit a note"), RequirePermissions(DiscordPermission.ModerateMembers)]
    ...
    [Command("Prune"),Description("Prune the message in the channel"),RequirePermissions(DiscordPermission.ManageMessages)]
    ...
    [Command("Prune"), SlashCommandTypes(DiscordApplicationCommandType.MessageContextMenu), RequirePermissions(DiscordPermission.ManageMessages)]
    ...
    [Command("Prune-User"), SlashCommandTypes(DiscordApplicationCommandType.MessageContextMenu), RequirePermissions(DiscordPermission.ManageMessages)]
    ...
    [Command("whitelist-invite"), Description("Whitelist an invite code"), RequirePermissions(DiscordPermission.ModerateMembers)]
    ...
    [Command("Message"), Description("Sends a message to specified user. Requires Mod Mail feature enabled."), RequirePermissions(DiscordPermission.ManageMessages)]
    ...
    [Command("Stats"),Description("Displays moderator stats for the server."),RequirePermissions(DiscordPermission.ManageMessages)]
    public async Task StatsAsync(CommandContext ctx)
    => await StatsCommand.ExecuteAsync(dbContextFactory, ctx);
    
    [Command("faq-create"), Description("Creates a new FAQ message"), RequirePermissions(DiscordPermission.ManageMessages)]
    public async Task CreateFaqAsync(SlashCommandContext ctx)
    => await CreateFaqCommand.ExecuteAsync(interactivity, ctx);

    [Command("faq-edit"), Description("Edits an existing FAQ message, using the message ID"), RequirePermissions(DiscordPermission.ManageMessages)]
    ...
    [Command("Say"), Description("Bot says a something"), RequirePermissions(DiscordPermission.ManageMessages)]
    ...
    [Command("timeout"), Description("Timeout a user"), RequirePermissions(DiscordPermission.KickMembers)]
